...
/obj/item/clothing/proc/return_inv()
	...
	for(var/obj/item/clothing/S in src)
		L += S.return_inv()
	return L


/obj/item/clothing/proc/show_to(mob/living/user)
...
/obj/item/clothing/proc/hide_from(mob/user)
	if(!user.client)
		return
	user.client.screen -= boxes
	user.client.screen -= closer
...
/obj/item/clothing/proc/close(mob/user)
	hide_from(user)
	user.s_active = null


/obj/item/clothing/proc/close_all() //returns 1 if any mobs actually got a close(M) call
...
/obj/item/clothing/proc/orient_objs(tx, ty, mx, my)
	var/cx = tx
	var/cy = ty
...
/obj/item/clothing/proc/standard_orient_objs(rows, cols, list/obj/item/display_contents)
	var/cx = 4
	var/cy = 2+rows
...
/obj/item/clothing/proc/orient2hud(mob/user)
...
/obj/item/clothing/proc/can_be_inserted(obj/item/W, stop_messages = 0)
...
/obj/item/clothing/proc/handle_item_insertion(obj/item/W, prevent_warning = 0)
	...
	if(usr)
		...
		if(!prevent_warning && !istype(W, /obj/item/weapon/gun/energy/crossbow))
			for(var/mob/M in viewers(usr, null))
				if(M == usr)
					usr << "<span class='notice'>Ты кладешь [W] в [src].</span>"
...
/obj/item/clothing/proc/remove_from_storage(obj/item/W, atom/new_location)
	if(!istype(W)) return 0
...
/obj/item/clothing/attackby(obj/item/W, mob/user)
	..()
...
/obj/item/clothing/attack_paw(mob/user)
	return attack_hand(user)

/obj/item/clothing/verb/toggle_gathering_mode()
...
/obj/item/clothing/verb/quick_empty()
...
/obj/item/clothing/New()
	...
	if(allow_quick_empty)
		verbs += /obj/item/clothing/verb/quick_empty
	else
		verbs -= /obj/item/clothing/verb/quick_empty

	if(allow_quick_gather)
		verbs += /obj/item/clothing/verb/toggle_gathering_mode
	else
		verbs -= /obj/item/clothing/verb/toggle_gathering_mode
...
/obj/item/clothing/attack_self(mob/user)
	...
	if(user.get_active_hand() == src)
		if(verbs.Find(/obj/item/clothing/verb/quick_empty))
			quick_empty()
		...
		if(!prevent_warning && !istype(W, /obj/item/weapon/gun/energy/crossbow))
			for(var/mob/M in viewers(usr, null))
				if(M == usr)
					...
					M.show_message("<span class='notice'>[usr] кладет [W] в [src].</span>")
				else if(W && W.w_class >= 3.0) //Otherwise they can only see large or normal items from a distance...
					M.show_message("<span class='notice'>[usr] кладет [W] в [src].</span>")
