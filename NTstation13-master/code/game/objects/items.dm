mob
	var //variables!
		...
		weight = 0
	Stat()
		statpanel("Inventory") // naming the statpanel
		...
		if(item&&(item in src)) //used for checking if the capsule, to open the tab.
			statpanel("Capsule")
			stat(item) //shows the capsule
//			stat("Weight Limit", item.limit) //shows the Capsule's weight limit
			stat("Weight Used", item.weight) ///shows the capsule's used weight
			stat("--Weight--","--Contained Item--") // seperator
			for(var/obj/x in item) //shows the contents of the capsule
...
obj
	...
	MouseDrop(turf/loca) // used for removing the item from the capsule
		if((loca in view(4))&&!(loca in usr)&&(isturf(loca))&&(src in usr.item)) // checking to see if the item is in the contents of the capsule, and if the location is within range
			src.loc = loca // setting the location
			usr.item.weight -= src.weight // subtracting the weight
	...
	MouseDown() //setting the mouse icon while dragging.
		...
		else if(src in usr.item) src.mouse_drag_pointer = src // setting the item in capsule while being dragged out
