	...
	switch(M.a_intent)
		if("help")
			...
			if((M.head && (M.head.flags & HEADCOVERSMOUTH)) || (M.wear_mask && (M.wear_mask.flags & MASKCOVERSMOUTH)))
				M << "<span class='notice'>Снимите маску!</span>"
				return 0
			...
			if(cpr_time < world.time + 30)
				...
				if((health >= -99 && health <= 0))
					...
					src << "<span class='unconscious'>Ты чувствуешь чье-то дыхание. Тебе немного лучше.</span>"
		...
		if("grab")
			...
			if(buckled)
				M << "<span class='notice'>Ты не можешь схватить [src], он/она пристегнут/а!</span>"
		...
		if("disarm")
			...
			if(randn <= 25)
				...
				playsound(loc, 'sound/weapons/thudswoosh.ogg', 50, 1, -1)
				visible_message("<span class='danger'>[M] роняет [src]!</span>",
								"<span class='userdanger'>[M] роняет [src]!</span>")
				forcesay(hit_appends)
				return
